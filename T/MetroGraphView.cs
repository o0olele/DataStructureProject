...
namespace T
{
    ...
    public partial class MetroGraphView : UserControl
    {
        ...
        private void PaintLineList(Graphics g)
        {
            ...
            //遮罩层
            Rectangle rc = new Rectangle(this.Width-480, 10, 480, (this.Graph.Lines.Count + 1) * 8);
            using (Brush brush = new SolidBrush(Color.FromArgb(60, Color.Gray)))
            {
            ...
            foreach (var line in this.Graph.Lines)
            {
                ...
                {
                    Brush bush = new SolidBrush(line.Color);//填充的颜色
                    g.DrawRectangle(pen, x, y, 10, 10);
                    g.FillRectangle(bush, x, y, 10, 10);
                }

                var sz = g.MeasureString(line.Name, this.Font);
                g.DrawString(line.Name, new Font("微软雅黑", 8, FontStyle.Bold), Brushes.Black, x + 20, y - sz.Height / 2 + 5);
        ...
        private void PaintLink(Graphics g, MetroLink link, bool IsCurLine)
        {
            ...
            if (IsCurLine)
            {
                pen = new Pen(link.Line.Color, 9);
            }
            else
            {
                pen = new Pen(link.Line.Color, 7);
            }
        ...
        private void PaintStartAndEndNodes(Graphics g)
        {
            ...
            if (this.StartNode != null)
            {
                ...
                int sx = this.StartNode.X - startNodeImage.Width / 2+5;
                int sy = this.StartNode.Y - startNodeImage.Height+14;
            ...
            if (this.EndNode != null)
            {
                ...
                int ex = this.EndNode.X - endNodeImage.Width / 2+5;
                int ey = this.EndNode.Y - endNodeImage.Height+14;
